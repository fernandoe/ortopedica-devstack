...
docker exec -i api-conta python manage.py shell -c "from django.contrib.auth import get_user_model; User = get_user_model(); User.objects.create_superuser('demo@example.com', 'password') if not User.objects.filter(email='demo@example.com').exists() else None;"
